...
pub const fn fib_faster(mut n: i32) -> i32 {
    let mut p1 = 0;
    let mut p2 = 1;
    while n > 1 {
        let t = p2;
...
pub fn combine(n: i32, k: i32) -> Vec<Vec<i32>> {
    let mut st: Vec<Vec<i32>> = (1..=n).map(|x| vec![x]).collect();
    let mut ans = vec![];
    while let Some(top) = st.pop() {
        ...
        let largest = top[top.len() - 1];
        for val in largest + 1..=n {
            let mut new_top = top.clone();
            new_top.push(val);
            st.push(new_top);
        }
...
pub fn max_subarray_sum(nums: Vec<i32>, k: i32) -> i64 {
    let mut ans = i64::MIN;
    let mut remainder = vec![i64::MAX; k as usize];
    remainder[0] = 0;
    let mut total = 0;

    for (i, &num) in nums.iter().enumerate() {
        total += num as i64;
        let idx = (i + 1) % k as usize;
        let val = remainder[idx];
        if val != i64::MAX {
            ans = ans.max(total - val);
        }
        remainder[idx] = val.min(total);
    }
